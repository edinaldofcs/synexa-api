...
export class DeadLetterProcessor {
  ...
  async process(job: Job<DeadLetterJobData>) {
    this.logger.warn(
      { original_queue: job.data.original_queue, original_job_id: job.data.original_job_id, reason: job.data.failed_reason },
    ...
    await this.prisma.outbox_events.create({
      data: {
        ...
        status: 'failed',
        error_message: job.data.failed_reason,
      },
    });

    this.logger.log({ original_job_id: job.data.original_job_id }, 'Dead-letter stored');
  }
}
