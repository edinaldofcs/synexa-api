...
import { JOB_INGEST_KNOWLEDGE_DOCUMENT, QUEUE_KNOWLEDGE } from '../queue.constants';
...
export class KnowledgeProcessor {
  ...
  async process(job: Job<KnowledgeJobData>) {
    this.logger.log({ document_id: job.data.document_id }, 'Ingesting knowledge document');
    await this.knowledgeService.ingestDocument(job.data.document_id);
    this.logger.log({ document_id: job.data.document_id }, 'Knowledge document ingested');
  }
}
