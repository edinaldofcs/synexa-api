...
export class IngestionProcessor {
  ...
  async process(job: Job<IngestJobData>) {
    const data = job.data;
    this.logger.log({ inbound_event_id: data.inbound_event_id }, 'Processing inbound event');
    ...
    for (const asset of mediaAssets) {
      if (asset.mime_type.startsWith('audio/') || asset.mime_type.startsWith('image/')) {
  ...
  private async resolveEndUser(data: IngestJobData): Promise<string> {
    ...
    await this.prisma.channel_identities.create({
      data: {
        ...
        channel_type: data.origin_channel,
        external_user_id: data.external_user_id,
        normalized_phone: data.origin_channel === 'whatsapp' ? data.external_user_id : null,
      },
    });
