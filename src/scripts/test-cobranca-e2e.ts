...
async function runTest() {
  console.log(
    '🧪 Inicializando contexto NestJS para Teste E2E do Fluxo de Cobrança...\n',
  );
  ...
  try {
    ...
    console.log(
      '💬 [Turno 1] Cliente: "Olá, meu CPF é 08334993942, gostaria de consultar minhas pendências."',
    );
    ...
    console.log(
      'Tool Calls:',
      JSON.stringify(
        response1.debug?.toolCalls?.map((t) => ({
          name: t.name,
          args: t.arguments,
          resultStatus: (t.result as any)?.status,
        })),
        null,
        2,
      ),
    ...
    console.log(
      ...
      JSON.stringify(response1.debug?.contextVariables, null, 2),
    );
    ...
    console.log(
      '💬 [Turno 2] Cliente: "Gostei da proposta NEG-001 à vista com 15% de desconto. Vamos fechar o acordo no PIX."',
    );
    const response2 = await chatService.send({
      ...
      originChannel: 'webchat_test',
      message:
        'Gostei da proposta NEG-001 à vista com 15% de desconto. Vamos fechar o acordo no PIX.',
    });
    ...
    console.log(
      'Tool Calls:',
      JSON.stringify(
        response2.debug?.toolCalls?.map((t) => ({
          name: t.name,
          args: t.arguments,
          resultStatus: (t.result as any)?.status,
        })),
    ...
    console.log(
      'Variáveis de Contexto:',
      JSON.stringify(response2.debug?.contextVariables, null, 2),
    );
