...
export class ApiAdapter implements ChannelAdapter {
  ...
  normalize(payload: Record<string, unknown>, headers?: Record<string, string>): NormalizedMessage {
    return {
      ...
      message: {
        type: ((payload.message as Record<string, unknown>)?.type as string) || 'text',
        text: ((payload.message as Record<string, unknown>)?.text as string) || undefined,
        parts: ((payload.message as Record<string, unknown>)?.parts as any[]) || undefined,
      },
  ...
  async send(connection: ChannelConnectionConfig, message: OutboundMessage): Promise<DeliveryResult> {
    ...
    if (!returnUrl) {
      this.logger.warn({ connectionId: connection.id }, 'API channel has no return_webhook_url to deliver response');
      return { success: true };
    }
