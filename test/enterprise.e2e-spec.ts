...
function signPayload(payload: Record<string, unknown>) {
  ...
  const signature = createHmac('sha256', HMAC_SECRET).update(`${timestamp}.${body}`).digest('hex');
...
describe('Enterprise Synexa (e2e)', () => {
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .useValue({
        ...
        releaseLock: jest.fn(),
        checkRateLimit: jest.fn().mockResolvedValue({ allowed: true, remaining: 99, resetAt: new Date() }),
        rpush: jest.fn(),
  ...
  describe('1. Pipeline de Mensagens', () => {
    beforeEach(() => {
      jest.spyOn(prisma.channel_connections, 'findUnique').mockResolvedValue(mockConn);
    });

    it('#1 POST /api/public/messages - texto simples → 202 queued', async () => {
      jest.spyOn(prisma.inbound_events, 'create').mockResolvedValue({
        id: 'evt-1', company_id: 'test-company',
        client_id: validPayload.client_id, channel_connection_id: null,
        channel_type: 'api', raw_payload: validPayload as any,
        request_id: null, normalized: false, status: 'received',
        error_message: null, idempotency_key: null, headers: null,
        created_at: new Date(), processed_at: null,
      });
    ...
    it('#2 Idempotencia - mesmo idempotency_key → 400', async () => {
      jest.spyOn(prisma.inbound_events, 'findFirst').mockResolvedValueOnce({ id: 'existing' } as any);

      const res = await signedPost('/api/public/messages', { ...validPayload, idempotency_key: 'dup-key' })
        .expect(400);
    ...
    it('#3 Canal invalido → 400', async () => {
      ...
      const res = await signedPost('/api/public/messages', { ...validPayload, origin_channel: 'sms' })
        .expect(400);
    ...
    it('#4 Client sem conexao ativa → 401 (HMAC falha sem connection)', async () => {
      jest.spyOn(prisma.channel_connections, 'findUnique').mockResolvedValue(null);

      const res = await signedPost('/api/public/messages', { ...validPayload, client_id: 'no-conn-client' });
  ...
  describe('2. Processamento do Agente', () => {
    it('#18 Imagem com vision - parte image enviada ao modelo', async () => {
      jest.spyOn(prisma.media_assets, 'create').mockResolvedValue({
        id: 'media-1', company_id: 'test-company', client_id: 'test-client',
        message_id: null, storage_bucket: null, storage_path: 'img/test.png',
        source_url: 'https://example.com/img.png', mime_type: 'image/png',
        file_size: 1024, checksum: null, duration_ms: null,
        width: null, height: null, transcript: null, ocr_text: null,
      ...
      jest.spyOn(prisma.messages, 'create').mockResolvedValue({ id: 'msg-img-1' } as any);
      jest.spyOn(prisma.inbound_events, 'create').mockResolvedValue({
        id: 'evt-img', company_id: 'test-company',
        client_id: validPayload.client_id, channel_connection_id: null,
        channel_type: 'api', raw_payload: payload as any,
        request_id: null, normalized: false, status: 'received',
        error_message: null, idempotency_key: null, headers: null,
        created_at: new Date(), processed_at: null,
      });
      ...
      const res = await signedPost('/api/public/messages', payload)
        .expect(ACCEPTED);
    ...
    it('#19 Imagem sem vision - fallback OCR usado', async () => {
      ...
      const { timestamp, signature } = signPayload({ ...validPayload, message: { type: 'mixed', parts: [{ type: 'image', url: 'https://example.com/img.png' }] } });
      const result = await request(app.getHttpServer())
        ...
        .send({
          ...validPayload,
          message: { type: 'mixed', parts: [{ type: 'image', url: 'https://example.com/img.png' }] },
        });
    ...
    it('#20 Audio sem suporte no modelo - fallback transcricao', async () => {
      const payload = {
        ...validPayload,
        message: { type: 'mixed', parts: [{ type: 'audio', url: 'https://example.com/audio.mp3', mime_type: 'audio/mpeg' }] },
      };

      jest.spyOn(prisma.messages, 'create').mockResolvedValue({ id: 'msg-audio-1' } as any);
      jest.spyOn(prisma.inbound_events, 'create').mockResolvedValue({
        id: 'evt-audio', company_id: 'test-company',
        client_id: validPayload.client_id, channel_connection_id: null,
        channel_type: 'api', raw_payload: payload as any,
        request_id: null, normalized: false, status: 'received',
        error_message: null, idempotency_key: null, headers: null,
      ...
      jest.spyOn(prisma.message_parts, 'create').mockResolvedValue({} as any);
      jest.spyOn(prisma.media_assets, 'create').mockResolvedValue({
        id: 'media-audio', company_id: 'test-company', client_id: 'test-client',
        message_id: 'msg-audio-1', storage_bucket: null, storage_path: null,
        source_url: 'https://example.com/audio.mp3', mime_type: 'audio/mpeg',
        file_size: null, checksum: null, duration_ms: null,
        width: null, height: null, transcript: null, ocr_text: null,
        status: 'pending', error_message: null,
        metadata: {} as any, created_at: new Date(), updated_at: new Date(),
      });
  ...
  describe('3. Webhooks', () => {
    it('#35 Webhook falha → retry agendado', async () => {
      jest.spyOn(prisma.webhook_endpoints, 'findMany').mockResolvedValue([
        { id: 'wh-1', client_id: 'c1', url: 'https://fail.example.com',
          events: ['message.completed'] as any, secret_hash: null,
          retry_policy: { max_retries: 3 } as any, enabled: true,
          channel_connection_id: null,
          created_at: new Date(), updated_at: new Date() },
      ]);
      jest.spyOn(prisma.webhook_deliveries, 'create').mockResolvedValue({
        id: 'del-1', webhook_endpoint_id: 'wh-1', event: 'message.completed',
        conversation_id: null, inbound_message_id: null,
        response_message_id: null, payload: {} as any,
        attempt: 1, max_attempts: 3, status: 'pending',
        http_status: null, response_body: null, error_message: null,
        next_retry_at: null, completed_at: null, created_at: new Date(),
      });
      jest.spyOn(prisma.webhook_deliveries, 'update').mockResolvedValue({} as any);

      jest.spyOn(prisma.channel_connections, 'findFirst').mockResolvedValue(mockConn);
      jest.spyOn(prisma.channel_connections, 'findUnique').mockResolvedValue(mockConn);
      jest.spyOn(prisma.conversations, 'findUnique').mockResolvedValue({
        id: 'conv-w1', company_id: 'test-company', client_id: 'test-client',
        status: 'active', mode: 'auto',
      } as any);
      jest.spyOn(prisma.inbound_events, 'create').mockResolvedValue({
        id: 'evt-w1', company_id: 'test-company',
        client_id: validPayload.client_id, channel_connection_id: null,
        channel_type: 'api', raw_payload: validPayload as any,
        request_id: null, normalized: false, status: 'received',
        error_message: null, idempotency_key: null, headers: null,
      ...
      jest.spyOn(prisma.messages, 'create').mockResolvedValue({ id: 'msg-out-1', status: 'completed' } as any);
      jest.spyOn(prisma.message_parts, 'create').mockResolvedValue({} as any);
      jest.spyOn(prisma.message_events, 'create').mockResolvedValue({} as any);
  ...
  describe('4. Auditoria', () => {
    it.skip('#26 Trace completo por request_id', async () => {
      jest.spyOn(prisma.inbound_events, 'findMany').mockResolvedValue([
        { id: 'ie-1', request_id: 'trace-1', status: 'received', raw_payload: {} as any } as any,
      ]);
      jest.spyOn(prisma.agent_runs, 'findMany').mockResolvedValue([
        { id: 'ar-1', request_id: 'trace-1', status: 'success', tool_calls: [] } as any,
      ]);
      jest.spyOn(prisma.tool_calls, 'findMany').mockResolvedValue([
        { id: 'tc-1', request_id: 'trace-1', tool_name: 'rag.search' } as any,
      ]);
      jest.spyOn(prisma.message_events, 'findMany').mockResolvedValue([
        { id: 'me-1', request_id: 'trace-1', event_type: 'message.created' } as any,
      ]);
    ...
    it.skip('#28 Filtro agent_runs por status', async () => {
      jest.spyOn(prisma.agent_runs, 'findMany').mockResolvedValue([
        { id: 'ar-fail', status: 'failed' } as any,
      ]);
  ...
  describe('5. Handoff', () => {
    it.skip('#22 Handoff em conversa já manual → 400', async () => {
      jest.spyOn(prisma.conversations, 'findUnique').mockResolvedValue({
        id: 'conv-manual', mode: 'manual', status: 'active',
      } as any);
    ...
    it.skip('#24 Liberar conversa automática → 400', async () => {
      jest.spyOn(prisma.conversations, 'findUnique').mockResolvedValue({
        id: 'conv-auto', mode: 'auto', status: 'active',
      } as any);
  ...
  describe('6. Observabilidade', () => {
    it.skip('#46 Latencia - retorna avg/p95', async () => {
      jest.spyOn(prisma.agent_runs, 'findMany').mockResolvedValue([
        { latency_ms: 100, status: 'success', model: 'gemini', started_at: new Date() },
        { latency_ms: 200, status: 'success', model: 'gemini', started_at: new Date() },
        { latency_ms: 300, status: 'failed', model: 'gemini', started_at: new Date() },
      ]);
    ...
    it.skip('#48 Erros por tenant', async () => {
      jest.spyOn(prisma.agent_runs, 'groupBy').mockResolvedValue([
  ...
  it('#43 Criacao de asset com mime type invalido → BadRequest', () => {
    ...
    const mediaService = app.get(require('../src/media/media.service').MediaService);
...
describe('8. Deprecacao', () => {
    it('/orchestrator/chat - headers de deprecated', async () => {
      jest.spyOn(prisma.channel_connections, 'findFirst').mockResolvedValue(null);
      jest.spyOn(prisma.channel_connections, 'findUnique').mockResolvedValue(null);
